...
% 3d spike removal alg
tStart = tic;
[xg, sg, costg] = watvDeSpeckle3D_GPU( y, J, K, mu, nit, param, GPUparam, dim );
disp(['time by watv gpu is ' num2str(toc(tStart))]);
residue = y - xg - sg;
x2 = xg + 0.1 .* residue;
...
fid = fopen([foldername filename '_deG.img'], 'w');
C = uint8(x2(:));
fwrite(fid, C);
fclose(fid);
