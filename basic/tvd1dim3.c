...
//gateway function for mex
{
    ...
    if (numOfDims != 3) {
        mexErrMsgTxt("Number of dimensions is not 3");
    }
    ...
    plhs[0]= mxCreateNumericArray(numOfDims,dims,mxDOUBLE_CLASS,mxREAL);
    ...
    /*Check that i index is a double scaler */
    if (mxIsDouble(prhs[2]) && mxGetNumberOfElements(prhs[2]) == 1) {
        iIndex = mxGetScalar(prhs[2]);
    }
    else
        mexErrMsgTxt("i index must be a double scalar.");
    
    /*Do TV along i-th dimension */
    if (iIndex == 1) {
        input1D = mxCalloc(m, sizeof(double));
        output1D = mxCalloc(m, sizeof(double));
        for (j = 0; j < n; j++) {
            for (k = 0; k < p; k++) {
                
                for (i = 0; i < m; i++) {
                    input1D[i] = input[i+j*m+k*m*n];
                }
                TV1D_denoise(input1D, output1D, m, lambda);
                for (i = 0; i < m; i++) {
                    output[i+j*m+k*m*n] = output1D[i];
                }
        ...
        input1D = mxCalloc(n, sizeof(double));
        output1D = mxCalloc(n, sizeof(double));
        for (i = 0; i < m; i++) {
            for (k = 0; k < p; k++) {
                
                for (j = 0; j < n; j++) {
                    input1D[j] = input[i+j*m+k*m*n];
                }
                TV1D_denoise(input1D, output1D, n, lambda);
                for (j = 0; j < n; j++) {
                    output[i+j*m+k*m*n] = output1D[j];
                }
        ...
        input1D = mxCalloc(p, sizeof(double));
        output1D = mxCalloc(p, sizeof(double));
        for (i = 0; i < m; i++) {
            for (j = 0; j < n; j++) {
                
                for (k = 0; k < p; k++) {
                    input1D[k] = input[i+j*m+k*m*n];
                }
                TV1D_denoise(input1D, output1D, p, lambda);
                for (k = 0; k < p; k++) {
                    output[i+j*m+k*m*n] = output1D[k];
                }
        ...
        mexErrMsgTxt("i index must be a 1 2 or 3");
