...
//gateway function for mex
{
    ...
    if (iIndex == 1) {
        ...
        for (j = 0; j < n; j++) {
            ...
            for (i = 0; i < m; i++) {
                input1D[i] = input[i+j*m];
            }
            TV1D_denoise(input1D, output1D, width, lambda);
            for (i = 0; i < m; i++) {
                output[i+j*m] = output1D[i];
            }
        }
        mxFree(input1D);
        mxFree(output1D);
        ...
        for (i = 0; i < m; i++) {
            ...
            for (j = 0; j < n; j++) {
                input1D[j] = input[i+j*m];
            }
            TV1D_denoise(input1D, output1D, width, lambda);
            for (j = 0; j < n; j++) {
                output[i+j*m] = output1D[j];
            }
        }
        mxFree(input1D);
        mxFree(output1D);
        ...
        mexErrMsgTxt("i index must be a 1 or 2");
